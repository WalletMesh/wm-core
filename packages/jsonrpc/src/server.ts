import type { JSONRPCMiddleware, JSONRPCRequest, JSONRPCResponse, RPCMethodMap, JSONRPCID } from './types.js';
...
export class JSONRPCServer<T extends RPCMethodMap> {
  private methods: Map<
    keyof T,
    (params: T[keyof T]['params']) => Promise<T[keyof T]['result']> | T[keyof T]['result']
  > = new Map();
  ...
  public registerMethod<M extends keyof T>(
    name: M,
    handler: (params: T[M]['params']) => Promise<T[M]['result']> | T[M]['result'],
  ): void {
   ...
   */
  public async receiveRequest(request: JSONRPCRequest<T, keyof T>): Promise<void> {
